...
EXTENSIONS_DIR='/app/tools/maratona/vscode-extensions/'
EXTENSIONS_OK='/app/tools/maratona/vscode-extensions/vscode-extensions-ok'
...
if [ -f "${EXTENSIONS_OK}" ]; then
  mkdir -p "${XDG_DATA_HOME}/vscode/extensions"
  cp --archive '/app/tools/maratona/vscode-extensions/' "${XDG_DATA_HOME}/vscode/extensions"
  msg "Got installed extensions"
  ...
  exec env PATH="${PATH}:${XDG_DATA_HOME}/node_modules/bin" \
    /app/extra/vscode/bin/code --extensions-dir="${EXTENSIONS_DIR}" "$@" ${WARNING_FILE}
fi
